...
dataNetwork <- function(target_nodes, df_edges, dict.combine, attrs){
  if(!is.null(df_edges)){
    ...
    df_nodes$nodetype[df_nodes$id %in% center_nodes] <- "target"
...
plot_network <- function(df_edges, target_nodes, hide_labels, 
  ...
  if(nrow(df_edges) > 0){
    ...
    legend_nodes <- df_nodes[, c("group1", "color.background")]
    legend_nodes <- legend_nodes[!duplicated(legend_nodes), ]
    legend_nodes$shape <- "ellipse"
    legend_nodes$font.color <- "black"
    legend_nodes$size <- 15
    colnames(legend_nodes) <- c("label", "color.background", "shape", "font.color", "size")
    
    legend_nodes$physics <- FALSE
    
    legend_nodes$font.size <- 12
